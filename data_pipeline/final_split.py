...
def compile_balanced_dataset():  
    jsonl_files = [file_name for file_name in os.listdir(PROCESSED_DIR) if file_name.endswith(".jsonl")]
    
    print("Loading tool files...")
    # get paths
    tool_paths = [os.path.join(PROCESSED_DIR, file_name) for file_name in jsonl_files if file_name != "NO_TOOL.jsonl"]
    ...
    # R = neg / (neg + pos)
    # neg = ( R / (1 - R) ) * pos
    no_tool_ds_size = int(tool_ds_size * (NEGATIVE_DATA_RATIO / (1 - NEGATIVE_DATA_RATIO)))

    if len(no_tool_ds) < no_tool_ds_size:
        print(f"Warning: Not enough negative samples to achieve ratio {NEGATIVE_DATA_RATIO}")
    ...
    ds_splits = master_ds.train_test_split(
        train_size = SPLIT_RATIO,
        seed=SEED
    )

    print(f"Exporting the clean train/test datasets...")
